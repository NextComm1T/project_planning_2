...
> 주: S0의 To-Be에 닉네임 설정 흐름을 추가했다 — 03-requirements.md R15(구 R33)·04-features.md F30 신설에 따른 반영(팀 확정 내용 그대로). 이후 "자동 랜덤 생성" 부분은 팀 결정으로 후순위 전환되어 "가입 시 직접 설정(필수)"만 남도록 다시 단순화했다.
...
> 주: 팀의 MVP 재축소 결정으로 목표 거리·시간 설정(S0)·종료 확인 메시지(S3)·주간·월간 랭킹 구분(S4·S5)까지 후순위로 뺐다. 주간·월간 랭킹 구분(및 종료 확인 메시지)은 03·04·05에도 이미 반영되어 있었고, 목표 거리·시간 설정만 반영이 늦어져 있었다 — 2026-09-10에 03(목표 거리·시간 설정 관련 항목, 구R1\~구R4)·04(F1\~F4)·05(P1)까지 후순위 전환을 마쳐 이제 02와 어긋나는 부분 없이 일치한다.
